...
> **2026年8月にスコープを再定義しました。** 以下のドキュメント群は離乳食トラッキング機能（M1〜M3）当時のものです。現行スコープ（食材在庫管理・大人用献立提案・週間献立表・ぴよログ連携）の設計指示書は別途作成されます。M1〜M3の実装自体は `archive/mealtracking-m1-m3` ブランチに保全されています（詳細はリポジトリルートの `README.md` 参照）。
