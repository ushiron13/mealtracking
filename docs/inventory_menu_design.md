...
- [ ] ④ぴよログ連携画面の実装（パース処理は実データサンプルを見て精緻化）※ダミー形式での実装は完了（ステップ5）。実データでの精度調整（ステップ6）は未着手
...
### 4.6 ぴよログ連携（ステップ5）のインポート履歴

`inventory_menu_claude_code_instructions.md` ステップ5は「直近のインポート履歴（実行日時、反映件数）の簡易表示」を求めているが、本ドキュメントのデータモデル（2章）にはインポート履歴を保持する専用テーブルの定義がない。そのため、今回の実装では**画面のコンポーネント状態のみで保持する（ページ再読み込みで消える、永続化しない）**簡易表示とした。永続化が必要な場合は、別途テーブル追加（例：`piyologImports: '++id, importedAt'`）を検討する。
